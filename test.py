...
# main
def modelRun(isClosed, start_time, end_time, closeTime, countsleep, issleep, set_value):
  #------------------- load model -------------------#
  ...
  while cap.isOpened():
    ...
    for face in faces:
      ...
      try:
          ...
          #cv2.rectangle(img, pt1=tuple(eye_rect_l[0:2]), pt2=tuple(eye_rect_l[2:4]), color=(255,0,0), thickness=1)
          #cv2.rectangle(img, pt1=tuple(eye_rect_r[0:2]), pt2=tuple(eye_rect_r[2:4]), color=(0,255,0), thickness=1)
          ...
          if (state_l[0]=='-') and (state_r[0]=='-'):
              ...
              if not isClosed:
                  ...
                  end_time = time.time()
          elif (state_l[0]=='O') and (state_r[0]=='O'):
              if isClosed:
          ...
          #------------------- alarm -------------------#
          if (closeTime > 3.0) and (countsleep == 3):  # 꾸벅꾸벅 졸음
              countsleep = 0
              speaker.ringAlarm()
          elif (closeTime > 10.0) :  # 잠
              # setIssleep(True)
              switch.switchControl(set_value)

              # 잔다고 판단하면 이 딥러닝 모델은 안돌려도 될듯?!
...
# modelRun(isClosed, start_time, end_time, closeTime, countsleep, issleep)